...
func (sf *Server) handleConnect(ctx context.Context, writer io.Writer, request *Request) error {
	...
	if sf.dialWithRequest != nil {
		...
		dial := sf.dial
		if dial == nil {
			dial = func(ctx context.Context, net_, addr string, addrSpec *statute.AddrSpec) (net.Conn, error) {
				return net.Dial(net_, addr)
			}
		}
		target, err = dial(ctx, "tcp", request.DestAddr.String(), request.DestAddr)
	}
	if err != nil {
		msg := err.Error()
...
func (sf *Server) handleAssociate(ctx context.Context, writer io.Writer, request *Request) error {
	...
	dial := sf.dial
	if dial == nil {
		dial = func(_ context.Context, net_, addr string, addrSpec *statute.AddrSpec) (net.Conn, error) {
			return net.Dial(net_, addr)
		}
	}
	...
	addr := bindLn.LocalAddr()
	udpAddr := addr.(*net.UDPAddr)
	ip := request.LocalAddr.(*net.TCPAddr)
	udpAddr.IP = ip.IP
	if err = SendReply(writer, statute.RepSuccess, udpAddr); err != nil {
		return fmt.Errorf("failed to send reply, %v", err)
	}
	...
	sf.goFunc(func() {
		...
		for {
			...
			if target, ok := conns.Load(connKey); !ok {
				...
				targetNew, err := dial(ctx, "udp", pk.DstAddr.String(), &pk.DstAddr)
				if err != nil {
